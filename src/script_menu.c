...
static void MultichoiceDynamicEventShowMon_OnSelectionChanged(struct DynamicListMenuEventArgs* eventArgs)
{
    ...
    u32 y = template->tilemapTop * 8 + 10;

    if (sMonSpriteId != MAX_SPRITES)
    {
        FreeSpriteTilesByTag(TAG_CB_MON_ICON);
        FreeSpritePaletteByTag(TAG_CB_MON_ICON);
        DestroySprite(&gSprites[sMonSpriteId]);
    }
    sMonSpriteId = CreateMonIcon(eventArgs->selectedItem, SpriteCB_MonIcon, 0, 0, 0, 0);
    gSprites[sMonSpriteId].oam.priority = 0;
    gSprites[sMonSpriteId].x = x;
    gSprites[sMonSpriteId].y = y;
}
...
void TryDrawRepelMenu(void)
{
    ...
    if (count > 1)
        DrawMultichoiceMenu(0, 0, 0, FALSE, menuPos);
